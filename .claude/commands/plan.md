---
description: Sequence the work. Work backwards from done.
---

Load `@fde` (`skills/fde/SKILL.md`). Stage: **plan**.

If `fde resume` shows NO ENGAGEMENT: ask "What should we call this client?" once, then **you** run `fde resume --init`. Never tell the human to type the CLI.

Run `fde resume` (fallback: `npx --yes fdeops resume`). Read `references/plan.md` and follow it. Sequence backwards from done, one small PR at a time, who signs. Each Now PR names `Kill if`.

Do not invent a plan the record cannot support. Missing success criteria → `unknown - ask: <question>`.

Confirm before any write to `.fde/`.

Done when: order, small PRs, and “done” are on the record and the human agrees.

Not for a one-line typo in an unbound file. Bound client work, a POC, eval, or go-live stays on `@fde`.
