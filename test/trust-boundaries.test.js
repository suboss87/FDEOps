...
test('startup emits one bounded canonical packet even for oversized engagement notes', t => {
  const f = fixture(t)
  fs.writeFileSync(path.join(f.eng, 'context.md'), '# Context\n**Phase:** land\n## Next action\nConfirm access\n' + '客户🙂'.repeat(100000) + '\n<private>HIDDEN_PACKET_SENTINEL</private>')
  const cli = f.run(['resume']), hook = f.hook()
  assert.equal(cli.status, 0, cli.stderr)
  assert.equal(hook.status, 0, hook.stderr)
  const packet = cli.stdout.trimEnd()
  assert.ok(Buffer.byteLength(packet) <= 16384)
  assert.ok(Buffer.byteLength(hook.stdout) <= 16384 + 512, 'startup wrapper must remain lean')
  assert.equal(hook.stdout.split(packet).length - 1, 1, 'one canonical packet per hook invocation')
  assert.doesNotMatch(hook.stdout, /HIDDEN_PACKET_SENTINEL|\uFFFD/)
})

test('entry packets identify their engagement and refresh after record or binding changes', t => {
  const f = fixture(t)
  fs.writeFileSync(path.join(f.eng, 'context.md'), '# Context\n## Next action\nINITIAL_ACTION\n')
  const first = f.hook()
  assert.equal(first.status, 0, first.stderr)
  assert.ok(first.stdout.includes(`ENGAGEMENT: ${f.eng}`))
  assert.match(first.stdout, /INITIAL_ACTION/)
  const unchanged = f.run(['resume'])
  assert.equal(unchanged.status, 0, unchanged.stderr)
  assert.ok(first.stdout.includes(unchanged.stdout.trimEnd()), 'unchanged entry has the same reusable packet')

  fs.writeFileSync(path.join(f.eng, 'context.md'), '# Context\n## Next action\nUPDATED_ACTION\n')
  const refreshed = f.run(['resume'])
  assert.equal(refreshed.status, 0, refreshed.stderr)
  assert.match(refreshed.stdout, /UPDATED_ACTION/)
  assert.doesNotMatch(refreshed.stdout, /INITIAL_ACTION/)
  const nextHook = f.hook()
  assert.equal(nextHook.status, 0, nextHook.stderr)
  assert.ok(nextHook.stdout.includes(refreshed.stdout.trimEnd()), 'new hook event must not be suppressed by cached entry')

  const other = path.join(f.dir, 'engagements', 'other', '.fde')
  fs.mkdirSync(other, { recursive: true })
  fs.writeFileSync(path.join(other, 'context.md'), '# Context\n## Next action\nOTHER_CLIENT_ACTION\n')
  const rebound = f.hook({ FDEOPS_ENGAGEMENT: other })
  assert.equal(rebound.status, 0, rebound.stderr)
  assert.ok(rebound.stdout.includes(`ENGAGEMENT: ${other}`))
