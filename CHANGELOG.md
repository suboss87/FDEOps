...
- `resume` / `triage`: a RECORD block - who signs off, what was promised, the last decisions - read from `stakeholders.md`, `delivery.md`, `decisions.md`. Monday's first screen no longer needs a second command to name the sponsor. Sealed notes stay sealed.
...
- Triage / status: an untouched engagement reads `new`. Green means asked and fine, never never-touched.
